...
class MarkovPasswordGenerator:
    ...
    def build_markov_models(self):
        ...
        self.all_chars = set(string.printable[:-6])  # 去除非打印字符，如空白和控制字符
        self.end_symbol = "\x00"  # 非常不常见的符号，作为结束符号
        self.all_chars.add(self.end_symbol)

        # 将计数转换为概率
        total_starts = sum(self.start_prob.values())
        for char in self.start_prob:
            self.start_prob[char] /= total_starts

        for order in range(1, self.max_order+1):
            for seq, transitions in self.markov_models[order].items():
                total_transitions = sum(transitions.values())
                for next_char in transitions:
                    self.markov_models[order][seq][next_char] /= total_transitions
